"""Erfassung der Dokumentenanforderungen eines Auftrags (Karte #316).
...
`auftrag.dokumentenanforderung` wird bereits von `progress.py` gelesen (offene
Punkte für angeforderte/offene Dokumente), lief bisher aber immer über eine
leere Liste — diese Seite ist die einzige Stelle, an der sich das füllt.
"""
...
@router.get("/auftrag/{auftrag_id}/unterlagen")
def unterlagen_form(request: Request, auftrag_id: str):
    ...
    return templates.TemplateResponse(
        request=request,
        name="auftrag/unterlagen.html",
        context={
            "auftrag": auftrag,
            "active_tab": "unterlagen",
            "active_nav": "auftrag",
            **sidebar_context
        }
    )
...
@router.post("/auftrag/{auftrag_id}/unterlagen")
async def unterlagen_submit(request: Request, auftrag_id: str):
    auftrag = storage.load_auftrag(auftrag_id)
    if not auftrag:
        return RedirectResponse(url="/auftrag", status_code=303)

    form_data = await request.form()
    auftrag.dokumentenanforderung = parse_unterobjekte(form_data, "dokument", Dokumentenanforderung)
    ...
    return RedirectResponse(url=f"/auftrag/{auftrag_id}/unterlagen", status_code=303)
