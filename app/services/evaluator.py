...
class EvaluatorService:
    ...
    def _extract_points(self, val: Any, feldef: Dict[str, Any]) -> float:
        """
        Extracts points for a field value according to schema.
        Unerfasste Kriterien (val is None, empty, unbekannt, rueckfrage) count as 0.0 points.
        Schema-abweichende Werte also count as 0.0 points.
    ...
    def _pick_worst_instance(self, inst_list: List[KriteriumBewertung]) -> KriteriumBewertung:
        ...
        return min(inst_list, key=lambda x: x.erreichte_punkte if x.erreichte_punkte is not None else 0.0)
    ...
    def calculate_objekt_status(self, obj: TechnikObjekt) -> str:
        ...
        for abschnitt in schema.get("abschnitte", []):
            for field in abschnitt.get("felder", []):
                ...
                sichtbar_cond = field.get("sichtbar_wenn")
                if sichtbar_cond:
